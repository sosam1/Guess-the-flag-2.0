...
function logicaDelJuego(modoDeJuego){
    ...
    //para que no se repitan las banderas//
    for(let i=0; i<paisesYaJugados.length; i++){
        if(paisesYaJugados[i] == pais_al_azar.name){
           pais_al_azar = countries[Math.floor(Math.random() * countries.length)];
        }
    } 
    ...
    let opciones = [pais_al_azar.name]
        ...
        for(let i=0;i<3;i++){
            let opcion = countries[Math.floor(Math.random() * countries.length)].name
    ...
    let opciones_container = document.getElementById("opciones-respuesta-container")
        ...
        for(let i=0; i<opciones.length; i++){
            ...
            <button id="opcion${i}" class="btn-opcion">${opciones_desordenadas[i]}</button>
    ...
    function logicaBotonesRespuesta(){

    //comparo cada boton con la respuesta correcta
    ...
    opcion0.addEventListener('click', function(){
        ...
        paisesYaJugados.push(pais_al_azar.name)//pusheo el pais al array
        console.log(paisesYaJugados)
